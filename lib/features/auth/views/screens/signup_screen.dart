...
class SignUpScreen extends ConsumerStatefulWidget {
  ...
  ConsumerState<SignUpScreen> createState() =>
      _SignUpScreenState(); // Create the corresponding state for the SignUpScreen
}
  ...
  void _signUp() async {
    ...
    if (_formKey.currentState!.validate()) {
      ...
      final authViewModel = ref.read(authViewModelProvider
          .notifier); // Access the Auth ViewModel for user management
      try {
        // Attempt to create a new user with the provided email and password
        await authViewModel.createUser(
          email:
              _emailController.text, // Retrieve email from the text controller
          password: _passwordController
              .text, // Retrieve password from the text controller
        );
      ...
      body: Center(
        child: Padding(
          padding: const EdgeInsets.symmetric(
              horizontal: 20), // Horizontal padding for the entire screen
          child: SingleChildScrollView(
            // Allows scrolling for the content
            child: Form(
                ...
                children: [
                  ...
                  Text(
                    ...
                    style: GoogleFonts.nunito(
                      // Use Google Fonts for custom text styling
                      color:
                          darkColor, // Set text color to darkColor defined in the theme
                      fontWeight: FontWeight.bold, // Make the font bold
                      fontSize: 50, // Set font size
                    ),
                  ),
                  const SizedBox(
                      height: 5), // Space between header and subtitle
                  Text(
                    'Join the Journey',
                    style: GoogleFonts.nunito(
                      // Subtitle styling
                  ...
                  ConstrainedBox(
                    constraints: const BoxConstraints(
                        maxWidth: 400), // Maximum width of 400 pixels
                    child: Column(
                      children: [
                        // Input field for user's name
                        QuranQuestTextField(
                          controller:
                              _nameController, // Bind to name controller
                          label: 'Name', // Label for the input field
                          validator: Validators
                              .validateName, // Validation function for name
                        ),
                        const SizedBox(
                            height: 10), // Space between input fields
                        // Input field for user's email
                        QuranQuestTextField(
                          controller:
                              _emailController, // Bind to email controller
                          label: 'Email', // Label for the input field
                          validator: Validators
                              .validateEmail, // Validation function for email
                        ),
                        const SizedBox(
                            height: 10), // Space between input fields
                        ...
                        QuranQuestTextField(
                          controller:
                              _passwordController, // Bind to password controller
                  ...
                  _isLoading
                      ? CircularProgressIndicator(
                          ...
                          color:
                              darkColor, // Set color of the loading indicator
                        )
                      : ConstrainedBox(
                          constraints: const BoxConstraints(
                          ...
                          child: CustomSubmitButton(
                            ...
                            onPressed:
                                _signUp, // Call sign-up method when pressed
                          ),
                        ),
